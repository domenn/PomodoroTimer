...
#include "test_util/make_application.h"
...
TEST(timersCorrect, startsAt25Minutes) {
    MockDateTime::setTime();
    Application application(ApplicationMode::POMODORO_TIMER);
    application.start();
    auto timer = application.getMainTimerValue();
...
TEST(timersCorrect, countdownCorrect) {
    ...
    ASSERT_EQ(con::as_minutes(application.getMainTimerValue()), 20);
    MockDateTime::addSeconds(12);
    ASSERT_EQ(QTime(0, 19, 48), QTime(0, 19, 48));
    auto t1 = con::as_qtime(application.getMainTimerValue());
    auto t2 = QTime(0, 19, 48);
    ASSERT_EQ(t1, t2);
}

TEST(switchCorrect, correctlySwitchesToPauseAndLongPause) {
    Application& app = (*mk::pomodoroTimerApp());
